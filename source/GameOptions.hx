...
class GameOptions extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		super.update(elapsed);
		    ...
		    if (curSelected == 14)
				versionShit.text = "Toggle auto pause when the game is unfocused.";
								...
								FlxFlicker.flicker(grpControls.members[curSelected], 1, 0.06, true, false, function(flick:FlxFlicker)
									{
										...
										switch(curSelected)
										{
											...
											case 14:
												grpControls.remove(grpControls.members[curSelected]);
												FlxG.save.data.AutoP = !FlxG.save.data.AutoP;
												var ctrl:Alphabet = new Alphabet(0, (80 * curSelected) + 60, (FlxG.save.data.instantRespawn ? 'autopause ON' : 'autopause OFF'), true, false);
												ctrl.y += 102;
												ctrl.x += 50;
												ctrl.targetY = curSelected - 14;
												grpControls.add(ctrl);
												FlxG.sound.play(Paths.sound('scrollMenu'));	
												if (FlxG.save.data.AutoP != null)
													FlxG.autoPause = FlxG.save.data.AutoP;		   	
										}
										//flick.destroy();
									});	
			}
